...
SpoofedConVar::SpoofedConVar(const char* var_name)
{
	m_cvar_restore = csgo::m_cvar->FindVar(var_name);
	Spoof();
}

SpoofedConVar::SpoofedConVar(ConVar* cvar)
{
	m_cvar_restore = cvar;
	Spoof();
}
...
SpoofedConVar::~SpoofedConVar()
{
	if (IsSpoofed())
	{
		...
		free(m_cvar_replace);
		m_cvar_replace = nullptr;
	}
}

bool SpoofedConVar::IsSpoofed()
{
	return (m_cvar_replace != nullptr);
}

void SpoofedConVar::Spoof()
{
	if (!IsSpoofed() && m_cvar_restore)
	{
		m_restore_flags = m_cvar_restore->m_nFlags;
		...
		std::memcpy(m_cvar_replace, m_cvar_restore, sizeof(ConVar));

		m_cvar_replace->m_pNext = nullptr;
		csgo::m_cvar->RegisterConCommand(m_cvar_replace);
...
{
	if (IsSpoofed())
		m_cvar_restore->m_nFlags = flags;
}

int SpoofedConVar::GetFlags()
{
	return m_cvar_restore->m_nFlags;
}

void SpoofedConVar::SetInt(int iValue)
{
	if (IsSpoofed())
		m_cvar_restore->SetValue(iValue);
}

void SpoofedConVar::SetBool(bool bValue)
{
	if (IsSpoofed())
		m_cvar_restore->SetValue(bValue);
}

void SpoofedConVar::SetFloat(float flValue)
{
	if (IsSpoofed())
		m_cvar_restore->SetValue(flValue);
}
